...
public class ThreadIteration6 extends Thread {
	...
	private void findCombinations9Weights() {
		...
		double c0 = a*charges[0] + b*charges[1] + c*charges[2] + d*charges[3] + e*charges[4] + f*charges[5];
		double c1 = 0;
		
		final List<Thread> threads = new ArrayList<Thread>();
		
		for (double g = mins[6]; g <= maxes[6]; g++) {
			s1 = s0 + g*doubleWeights[6];
			c1 = c0 + g*charges[6];
			
			// create threads here for each value of b
			final ThreadIteration7 iteration = new ThreadIteration7(
