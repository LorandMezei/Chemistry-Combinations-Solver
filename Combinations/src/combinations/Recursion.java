	...
	public void findCombinationsLowerMatch(final List<String> weights, 
		...
		private String getFormula(final List<String> path,
	        ...
	        for (int i = 0; i < entrySet.size(); i++) {
	        	...
	        	final Integer charge = ComponentsService.getWeightsToCharges().get(weight);
	        	...
	        	if (ComponentsService.getWeightsToSymbols().containsKey(weight)) {
	        		final String symbol = ComponentsService.getWeightsToSymbols().get(weight);
	        		if (symbol.length() > 2) {
	        			formula += "(" + symbol + ")" + subscript;
	        		} else {
	        			formula += ComponentsService.getWeightsToSymbols().get(weight) + subscript;
	        		}
		...
		private boolean areAllComponentsInRange(final String solution,
			...
			for (int i = 0; i < weights.size(); i++) {
				final String symbol = ComponentsService.getWeightsToSymbols().get(new BigDecimal(weights.get(i)));
