...
public class ComponentsService {
			...
			while (line != null) {
			    ...
			    if (values.size() == NUM_FILE_ARGUMENTS) {
			    	...
			    	if (values2.get(1).length() == 4) {
			    		weight = new BigDecimal(values.get(WEIGHT_FILE_INDEX)).setScale(4, RoundingMode.CEILING);
			    	} else {
			    		weight = new BigDecimal(values.get(WEIGHT_FILE_INDEX)).setScale(3, RoundingMode.CEILING);
			    	}
	...
	public static List<String> getSymbols() {
		...
		for (final Map.Entry<String, BigDecimal> entry : ComponentsService.getSymbolsToWeights().entrySet()) {
